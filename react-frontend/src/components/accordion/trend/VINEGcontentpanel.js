...
import { nForVINEG } from '../../../redux'
import { Grid, Form, Select} from "semantic-ui-react"

function VINEGcontentPanel(props) {
            ...
            <Grid.Column width={4}>
                <Form.Field
                    ...
                    onChange ={(e,selectedOption) => {
                        props.nForVINEGdispatch(selectedOption.value)
                        }}
                />
            </Grid.Column>
        </Grid.Row>
...
const mapStateToProps = state => {
  return {
    nForVINEG: state.trendFromRootReducer.nForVIPOS,
  }
}

const mapDispatchToProps = dispatch => {
  return {
    nForVINEGdispatch: x => dispatch(nForVINEG(x)),
  }
}
