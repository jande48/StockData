...
import { nForKeltnerC } from '../../../redux'
import { Grid, Menu, Form, Select} from "semantic-ui-react"

function KCcontentPanel(props) {
  ...
  const nclass = new createTiles('Period (n):','20',props.nForKCdispatch)

  const objectList = [nclass]
...
const mapDispatchToProps = dispatch => {
  return {
    nForKeltnerCdispatch: x => dispatch(nForKeltnerC(x)),
